$(function() {
  ...
  $('.movie-button').on('click',function(){
    var my_api_key = '2c4a17a043d93c60bd7f3fd7b0c00958';
    var title = $('.movie').val();
    if (title=="") {
...
function movie_recs(movie_title,movie_id,my_api_key){
  $.ajax({
    ...
    success: function(recs){
      if(recs=="OOps! requested movie is not in our database. Please check the spelling or try with some other movies"){
        $('.fail').css('display','block');
        $('.results').css('display','none');
        $("#loader").delay(500).fadeOut();
      }
